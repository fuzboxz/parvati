...
- **±127 clamp & 1/128-semitone quantization**: enforced at every writer (`ScalaImport.cpp:412-419`; `SynthEngine.cpp:1044-1046`; `TuningEditor.cpp:306-308`; `HellcatPreset.cpp:924-933`). Any new path must clamp identically and surface the clamp warning (G5).
...
- **D4 invariant**: custom active ⇒ raga byte 0 (`SynthEngine.h:196-201`; `HellcatPreset.cpp:935`). G3's loop must go through `setPartTuningCustom`; a raw byte-4 write would shadow the custom table.
