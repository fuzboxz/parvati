...
int main()
{
    ...
    std::printf ("\n");
    check (descs.size() == 183, "exposes exactly 183 parameters (108 patch/part + 5 arp + 3 options + 67 sequencer)");
    check (groups["osc"] == 8, "8 oscillator params");
    check (groups["mix"] == 8, "8 mixer params");
    check (groups["filter"] == 9, "9 filter params (2x cutoff/reso/mode + env + lfo + filter_card)");
