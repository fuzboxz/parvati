...
//
//   ./build_unified/hellcat_unified_tests ipad_hig_sizing_test
...
#include "PluginEditor.h"          // HellcatEditor (kHeaderH/kBarHeight) + ParamPage (kMargin)
...
#include "ui/HellcatLookAndFeel.h" // kPopupRowHeight (default popup rows)
...
static_assert (HellcatEditor::kHeaderH   == 52, "header height must be 52 (44pt HIG strip + 8px chrome depth)");
static_assert (HellcatEditor::kBarHeight == 44, "header icon strip must be 44 (HIG targets)");
...
static_assert (HellcatLookAndFeel::kPopupRowHeight == 44, "default popup-menu rows must be 44 (HIG)");
...
TEST(ipad_hig_sizing_test)
{
    ...
    check (HellcatEditor::kHeaderH    == 52, "header height == 52 (44 HIG strip + chrome depth)");
    check (HellcatEditor::kBarHeight  == 44, "header icon strip == 44");
