...
using hellcat::fv1::Fv1Echo;
using hellcat::fv1::Fv1ClockedDelay;
using hellcat::fv1::Fv1Flanger;
using hellcat::fv1::Fv1Phaser;
using hellcat::fv1::Fv1PlateReverb;
using hellcat::fv1::Fv1Room;
using hellcat::fv1::Fv1Spring;
using hellcat::fv1::Fv1Chorus;
using hellcat::fv1::Fv1Ensemble;
using hellcat::fv1::Fv1JunoChorus;
using hellcat::fv1::Fv1LutDistortion;
using hellcat::fv1::Fv1Overdrive;
...
TEST(hellcat_fx_invariants_test)
{
    ...
    std::printf ("[I2b] feedback loops do not amplify near-Nyquist content\n");
    {
        ...
        auto nyqGain = [&all] (int kind) -> double
        {
            ...
            auto runFx = [&in, &out] (auto& f)
            {
                for (int i = 0; i < n; ++i)
                {
                    const int32_t x = hellcat::fv1::f24_fromFloat (in[(size_t) i]);
                    int32_t lo = 0, ro = 0;
                    f.processSampleFx (x, x, lo, ro);
                    out[(size_t) i] = hellcat::fv1::f24_toFloat (lo);
                }
            };
            switch (kind)
            {
