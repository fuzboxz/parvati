...
// HELLCAT_TEST_SHOTS=1, mirroring HELLCAT_TEST_HOLD) saves PNG snapshots to
...
void snapshot (juce::Component& c, const juce::String& name)
{
    ...
    if (juce::SystemStats::getEnvironmentVariable ("HELLCAT_TEST_SHOTS", "0") != "1")
        return;
...
TEST(hellcat_settings_probe)
{
    ...
    const bool interactive = juce::SystemStats::getEnvironmentVariable ("HELLCAT_TEST_HOLD", "0") == "1";
    ...
    win->setVisible (false);
    delete ed;
    std::printf ("PROBE DONE (PNG shots in /tmp/settings_shots only when HELLCAT_TEST_SHOTS=1)\n");
    return true;
}
