...
void render (HellcatAudioProcessor& proc, [[maybe_unused]] double sr, int bufSize,
...
TEST(hellcat_fx_dropout_probe)
{
    ...
    // sections are gated on HELLCAT_TEST_HOLD so the default run is
    // deterministic and terminates.
    const double sr = 48000.0;
    int bufSize = 512;
    const bool extended = std::getenv ("HELLCAT_TEST_HOLD") != nullptr;
    ...
    // 100% everything (drive/fold/bias/tone + 100% wet).
    {
        ...
        for (const auto& r : repro)
        {
            ...
            if (std::string (r.name).find ("Overdrive") != std::string::npos)
                setEnvVar ("HELLCAT_TAP_ON", "1");
            render (proc, sr, bufSize, capL, capR);
            setEnvVar ("HELLCAT_TAP_ON", "");
            const Metrics a = analyze (capL, (int) (0.3 * sr));
