// Hellcat-native YAML preset format round-trip + forward-compat verification.
//
// Proves the .yml format carries EVERYTHING Hellcat can do — including the
// Hellcat-only `vca_curve` / `filter_card` options and the arp settings that the
// Ambika .PRO byte format silently drops. A patch sculpted with the SVF +
// exponential VCA survives a .yml save+load intact, whereas a .PRO of the
// same state reverts them.
//
// Run: ./build_unified/hellcat_unified_tests hellcat_preset_test
...
TEST(hellcat_preset_test)
{
    ...
    std::printf ("[1] YAML emit/parse round-trips a var tree\n");
    {
        ...
        root->setProperty ("format", "hellcat-patch");
        ...
        const juce::String text = emitHellcatYaml (juce::var (root.release()));
        const juce::var parsed = parseHellcatYaml (text);
    ...
    // ---------------------------------------------------------------------
    std::printf ("\n[2] .yml patch round-trip: vca_curve/filter_card/arp survive\n");
    {
        HellcatAudioProcessor a, b;
        a.prepareToPlay (48000.0, 512);
        b.prepareToPlay (48000.0, 512);

        // Set several params, INCLUDING the Hellcat-only options + arp that .PRO
        ...
        check (yaml.contains ("hellcat_version: 0.1.0"), "hellcat_version is 0.1.0 (pre-release)");
        ...
        check (mism == 0, "end-to-end .yml: EVERY APVTS value matches (incl options/arp)");
    ...
    std::printf ("\n[3] Contrast: .PRO of the same state DROPS the options\n");
    {
        ...
        const auto tmp = juce::File::getSpecialLocation (juce::File::tempDirectory)
                             .getChildFile ("hellcat_preset_gap.PRO");
    ...
    std::printf ("\n[4] Forward-compat: unknown keys + typo'd paramIDs are ignored\n");
    {
        ...
        check (applyHellcatPatch (proc, yaml), "load succeeds with unknown keys");
    ...
    // ---------------------------------------------------------------------
    std::printf ("\n[5] .yml MULTI round-trip (all 6 parts + routing)\n");
    {
        HellcatAudioProcessor a, b;
        a.prepareToPlay (48000.0, 512);
        b.prepareToPlay (48000.0, 512);
        ...
        // configDirty_, and serializeHellcatMulti reads pendingConfig_.)
        ...
        check (yaml.contains ("format: hellcat-multi"), "multi YAML has format tag");
    ...
    // ---------------------------------------------------------------------
    std::printf ("\n[6] .yml PATCH load: part_raga is the whole tuning state\n");
    {
        ...
        juce::File f = juce::File::getSpecialLocation (juce::File::tempDirectory)
                           .getChildFile ("hellcat_t1_patch.yml");
        check (f.replaceWithText (
                   "format: hellcat-patch\nversion: 1\nname: \"T\"\nparams:\n"
                   "  part_raga: 0\n"),
               "T1 patch file written");
        check (p.loadHellcatPatchFile (f), "T1: .yml patch loads");
    ...
    // ---------------------------------------------------------------------
    std::printf ("\n[7] .yml MULTI load: no tuning keys emitted; legacy keys accepted\n");
    {
        ...
        // tuning_mode still load (custom 33 -> 12-EDO) — see hellcat_tuning_test
        ...
        juce::File f = juce::File::getSpecialLocation (juce::File::tempDirectory)
                           .getChildFile ("hellcat_t2_multi.yml");
        check (a.saveHellcatMultiFile (f), "T2: multi saved with raga 3 on part 0");
        {
            juce::String text;
        ...
        check (b.loadHellcatMultiFile (f), "T2: .yml multi loads");
    ...
    std::printf ("\n[T3] Wave 2: degenerate multi docs rejected; missing routing keys default\n");
    {
        ...
        juce::File empty = juce::File::getSpecialLocation (juce::File::tempDirectory)
                               .getChildFile ("hellcat_t3_empty.yml");
        empty.replaceWithText ("format: hellcat-multi\nversion: 1\nname: \"Empty\"\nparts: []\n");
        b.getEngine().setPartVoiceSlots (0, 11);   // distinctive pre-state
        b.getEngine().setPartName (0, " sentinel");
        check (! b.loadHellcatMultiFile (empty), "T3: empty parts array rejected");
        ...
        juce::File scalars = juce::File::getSpecialLocation (juce::File::tempDirectory)
                                 .getChildFile ("hellcat_t3_scalars.yml");
        scalars.replaceWithText ("format: hellcat-multi\nparts:\n  - 7\n  - 9\n");
        check (! b.loadHellcatMultiFile (scalars), "T3: non-object parts entries rejected");
        ...
        juce::File sparse = juce::File::getSpecialLocation (juce::File::tempDirectory)
                                .getChildFile ("hellcat_t3_sparse.yml");
        ...
        check (b.loadHellcatMultiFile (sparse), "T3: sparse multi loads");
    ...
    std::printf ("\n[T4] Wave 2: quoted top-level names round-trip\n");
    {
        ...
        juce::File f = juce::File::getSpecialLocation (juce::File::tempDirectory)
                           .getChildFile ("hellcat_t4_name.yml");
        check (a.saveHellcatMultiFile (f), "T4: multi with hostile name saved");
        check (b.loadHellcatMultiFile (f), "T4: multi with hostile name LOADS (was a silent parse failure)");
        // NOTE: the loaded program name comes from the FILENAME on this path
        // (loadHellcatMultiFile's tail), not the document — the escaping
        ...
        // must never have entered the line-based document at all.
        {
            ...
            const juce::var re = parseHellcatYaml (saved);
    ...
    std::printf ("\n[T5] Wave 2: part NAMES lifecycle across loads\n");
    {
        ...
        const juce::File drum = tplDir.getChildFile ("Drum Kit (GM).yml");
        if (drum.existsAsFile())
        {
            check (p.loadHellcatMultiFile (drum), "T5: Drum Kit loads");
            ...
            // always emits them).
            check (p.loadHellcatMultiFile (drum), "T5: Drum Kit re-loads");
            check (eng.getPartName (0) == "Kick", "T5: names restored from the .yml multi");
            ...
            std::printf ("     (Drum Kit template not found — run hellcat_gen_templates)\n");
    }
    ...
    // ---------------------------------------------------------------------
    std::printf ("\n[T7] Wave 2: corrupt .yml PATCH leaves voices alone\n");
    {
        HellcatAudioProcessor p;
        ...
        // A corrupt .yml PATCH (the multi path got validate-first; the
        ...
        juce::File bad = juce::File::getSpecialLocation (juce::File::tempDirectory)
                             .getChildFile ("hellcat_t7_bad.yml");
        bad.replaceWithText ("this is not: yaml: at all: [");
        check (! p.loadHellcatPatchFile (bad), "T7: corrupt patch rejected");
        bad.deleteFile();
    ...
    // ---------------------------------------------------------------------
    std::printf ("\n[T8] Wave 3: hand-edited .yml routing values are clamped\n");
    {
        ...
        juce::File f = juce::File::getSpecialLocation (juce::File::tempDirectory)
                           .getChildFile ("hellcat_t8_clamp.yml");
        f.replaceWithText (
            "format: hellcat-multi\nversion: 1\nname: \"Clamp\"\nparts:\n"
            "  - channel: 17\n"
            "    keyzone_low: 100\n"
            "    keyzone_high: 20\n"
            "    voice_slots: 4\n");
        check (p.loadHellcatMultiFile (f), "T8: clamped multi loads");
    ...
    std::printf ("\n[T9] W11: options: accepts ONLY option descriptors (F-state-5)\n");
    {
        ...
        juce::File f = juce::File::getSpecialLocation (juce::File::tempDirectory)
                           .getChildFile ("hellcat_t9_options.yml");
        check (a.saveHellcatMultiFile (f), "T9: reference multi saved");
        juce::String txt;
        if (juce::FileInputStream in (f); in.openedOk())
        ...
        check (b.loadHellcatMultiFile (f), "T9: multi with hostile options keys loads");
