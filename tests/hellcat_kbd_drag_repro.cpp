...
TEST(hellcat_kbd_drag_repro)
{
...

    {
        auto proc = std::make_unique<HellcatAudioProcessor>();
        proc->prepareToPlay (sr, 256);
        // Pin the Ladder card: the 0.05 crackle gate was calibrated on its
        // smoothed retrigger transient. The card's step response is card
        // character (an OTA cascade passes a sharper retrigger edge); this
        // test guards the VCA crackle regression, not the card.
        ...
        for (int w = 0; w < total; )
        {
            ...
            {
                // ~75 keys/s upward sweep: retarget every ~13 ms (~5.7
                // blocks). CLAMP at 127: the old 60..135 sweep overflowed the
                // MIDI range (a JUCE assert fired and the note wrapped to 7,
                // a subsonic retarget the sweep never intended).
                const double tIn = (w - dragStart) / sr;
                const int note = juce::jmin (127, 60 + (int) (tIn * 75.0));
                if (note != curNote)
                {
