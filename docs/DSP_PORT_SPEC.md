...
There is **no firmware filter**. The voice computes `cutoff` (8-bit, 0..127 via `U14ShiftRight6`, voice.cc:274) and `resonance` (8-bit, voice.cc:276-278) once per block, plus `mode` (`patch.filter[0].mode`, written to hardware in voicecard.cc:143). Parvati emulates 5 selectable topologies:
