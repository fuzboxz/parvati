...
| IR3109 (card index 5) | 4-pole OTA cascade (Juno-60/106 class) | custom model (this repo): the SAME structure as the SMR4 card, a calibrated sibling — not a new structure | LP only. Three character deltas, all test-pinned: a higher stage knee (0.10 vs 0.052: about 10 dB less 3rd harmonic in the weak zone — the filter stays polite until driven), a milder resonance-feedback clip (3x the knee), and the resonance factor capped at 3.4, BELOW the exact 4.0 onset: the factory cap, this card never self-oscillates (the famous Juno character; the seeded ring at resonance 1.0 decays). Honest note: the Juno sound also owes much to its BBD chorus, which a filter card cannot carry (a chorus FX is possible future work). |

**Decision:** expose a single `AnalogFilter` class with an enum `Topology { FOUR_POLE_LADDER, FOUR_POLE_SSM2164, TWO_POLE_SVF, FOUR_POLE_OTA, TWO_POLE_POLIVOKS, FOUR_POLE_IR3109 }` (selectable per-patch or globally). For v1, all three original topologies use LP; they differ in slope/pole-count via the chosen JUCE class. (LM13700 vs SSM2164 differ subtly in resonance drive/saturation. V1 treats both as the JUCE ladder — a follow-up enhancement can add OTA saturation.) The 2026-08-24 follow-up added `FOUR_POLE_OTA`: a from-scratch SMR4 OTA-cascade model (four `gm*tanh(error/2Vt)` stages, trapezoidal integration with a one-step Newton solve, linearised delay-free resonance loop with its own VCA soft clip). The same date added `TWO_POLE_POLIVOKS`: a from-scratch Soviet Polivoks-form SVF (op-amp character layer on the exact `R = 2*(1-res)` skeleton — asymmetric hard-shoulder rails, diode-style resonance limiting, damping sag, input offset, rate-limited outputs, supply clamp; LP/BP outputs; onset exactly at resonance 1.0). The same day added `FOUR_POLE_IR3109`: the Juno-60/106-class sibling of the SMR4 — the same OTA-cascade structure with three calibration deltas (knee 0.10, milder feedback clip, kfb capped at 3.4 below the exact 4.0 onset so it never self-oscillates). A user draft provided the structure and independently stated the 4.0 onset; the shipped card reuses the machine-precision-verified SMR4 engine.
