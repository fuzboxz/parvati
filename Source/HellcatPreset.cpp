...
// crafted .yml with thousands of increasing-indent lines would recurse
...
float currentRaw (HellcatAudioProcessor& proc, const PatchParamDescriptor& d)
{
...
// from engine storage. Patch/Part byte params use hellcatPatchByteToValue; arp
...
float partRaw (SynthEngine& engine, int partIndex, const PatchParamDescriptor& d)
{
    ...
    if (d.isFx)
    {
        ...
        // patchBytes[-1] (byteOffset=-1) and crash on .yml multi save.
    ...
    return hellcatPatchByteToValue (d, byte);
}

// Build a `params:` map (DynamicObject) for the current part.
std::unique_ptr<juce::DynamicObject> currentParamsMap (HellcatAudioProcessor& proc)
{
    auto obj = std::make_unique<juce::DynamicObject>();
...
{
    const var tree = parseHellcatYaml (yaml);
...
juce::String serializeHellcatMulti (HellcatAudioProcessor& proc)
{
    ...
    juce::String out;
    out << "# Hellcat multi — human-editable. 6-part multitimbral setup.\n";
    ...
    for (int i = 0; i < SynthEngine::getNumParts(); ++i)
    {
        ...
        // Hellcat extension: the per-part voice count (1..16; 0 = disabled
        ...
        // files that carry them still load (see applyHellcatMulti).
...
bool applyHellcatMulti (HellcatAudioProcessor& proc, const juce::String& yaml)
{
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        // Hellcat extension: per-part voice slots + name. The slots are the
        ...
        if (auto* pobj = pmap.getDynamicObject())
        {
            for (const auto& p : pobj->getProperties())
            {
                ...
                if (d->isArp)
                {
                    ...
                    // bounds) BEFORE staging — the same rule applyHellcatPatch
                    ...
                    // hellcat_load_invariants_test at authoring time).
    ...
    if (auto* oobj = options.getDynamicObject())
    {
        ...
        // capacity mode. Keep PARSING it so older .yml files still load, but
        ...
        if (oobj->hasProperty ("voice_mode"))
        {
            ...
            // older .yml multis still load. Do NOT call setUiVoiceMode.
