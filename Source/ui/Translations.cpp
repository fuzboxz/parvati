...
const juce::String& frenchChromeStrings()
{
    static const juce::String text = juce::CharPointer_UTF8 (
        ...
        "\"Load Parvati Patch (.parvati)\" = \"Charger un patch Parvati (.parvati)\"\n"
        "\"Export .MUL\" = \"Exporter .MUL\"\n"
        "\"Export the current part as an Ambika .PRO patch \" = \"Exporte la partie actuelle en patch Ambika .PRO \"\n"
        "\"(byte-faithful, hardware-shareable). Parvati-only options \" = \"(fidèle au format, partageable avec le matériel). Les options Parvati \"\n"
        "\"(VCA curve, filter card, arp) are not carried — use Save \" = \"(courbe VCA, carte filtre, arp) ne sont pas reprises — utilisez Enregistrer \"\n"
        "\"(.parvati) for the full patch.\" = \"(.parvati) pour le patch complet.\"\n"
        ...
        "\"voicecards, the export-fallback dialog maps them onto the \" = \"voicecards, le dialogue de repli d'export les répartit sur les \"\n"
        "\"6 cards.\" = \"6 cartes.\"\n"
        "\"More\" = \"Plus\"\n"
        "\"Channel\" = \"Canal\"\n"
        ...
        "\"channel (multitimbral stacks usually want distinct channels).\" = \"les canaux (les stacks multitimbraux préfèrent des canaux distincts).\"\n"
        ...
        "\"below stay silent so another part can own them.\" = \"notes plus basses restent muettes pour qu'une autre partie les possède.\"\n"
        ...
        "\"above stay silent so another part can own them.\" = \"notes plus hautes restent muettes pour qu'une autre partie les possède.\"\n"
        "\"Zone High\" = \"Zone (haut)\"\n"
...
const juce::String& germanChromeStrings()
{
    static const juce::String text = juce::CharPointer_UTF8 (
        ...
        "\"Load Parvati Patch (.parvati)\" = \"Parvati-Patch (.parvati) laden\"\n"
        "\"Export .MUL\" = \".MUL exportieren\"\n"
        "\"Export the current part as an Ambika .PRO patch \" = \"Exportiert den aktuellen Part als Ambika-.PRO-Patch \"\n"
        "\"(byte-faithful, hardware-shareable). Parvati-only options \" = \"(bytetreu, hardwarekompatibel). Parvati-exklusive Optionen \"\n"
        "\"(VCA curve, filter card, arp) are not carried — use Save \" = \"(VCA-Kurve, Filterkarte, Arp) werden nicht übernommen — nutze Speichern \"\n"
        "\"(.parvati) for the full patch.\" = \"(.parvati) für den vollständigen Patch.\"\n"
        ...
        "\"channel (multitimbral stacks usually want distinct channels).\" = \"jedem Kanal (multitimbrale Setups wollen meist eigene Kanäle).\"\n"
        "\"Key zone: the lowest MIDI note this part responds to. Notes \" = \"Tastenzone: tiefste MIDI-Note, auf die diese Part reagiert. \"\n"
        "\"below stay silent so another part can own them.\" = \"Tiefere Noten bleiben stumm, damit sie eine andere Part übernehmen kann.\"\n"
        "\"Key zone: the highest MIDI note this part responds to. Notes \" = \"Tastenzone: höchste MIDI-Note, auf die diese Part reagiert. \"\n"
        "\"above stay silent so another part can own them.\" = \"Höhere Noten bleiben stumm, damit sie eine andere Part übernehmen kann.\"\n"
        "\"Zone High\" = \"Zone (hoch)\"\n"
        "\"Oct\" = \"Okt\"\n"         // abbreviation (Oktave)
