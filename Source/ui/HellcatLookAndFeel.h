...
//
// HellcatLookAndFeel — a juce::LookAndFeel_V4 subclass that restyles every
// stock component from the active HellcatTheme. setTheme() stores the theme
...
constexpr int hellcatTabCategoryColourId = 0x2F000001;
...
class HellcatModuleLamp : public juce::Button
{
public:
    HellcatModuleLamp() : juce::Button ({}) { setClickingTogglesState (false); }
...
private:
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (HellcatModuleLamp)
};
...
public:
    ...
    /** Points theme_ at @p t and applies setColour() for every standard colour
        ...
        change). */
    void setTheme (const HellcatTheme& t);

    /** The active theme, or nullptr if setTheme() has never been called. */
    const HellcatTheme* getTheme() const noexcept { return theme_; }
    ...
    // one UI family. The ASCII "HELLCAT" logo keeps its own monospaced font
    // (see HellcatEditor::paint) and is NOT routed through appFont().
    ...
    // / shadow, EXCEPT buttons carrying the "hellcatButtonOutlined" component
...
private:
    const HellcatTheme* theme_ = nullptr;
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (HellcatLookAndFeel)
};
...
// the hellcat:: fallback colour constants from HellcatTheme.h. Every
...
bool isY2kTheme (const HellcatTheme* t) noexcept;
...
void paintChromeCard (juce::Graphics& g, const juce::Rectangle<float>& r,
                      float corner, const HellcatTheme* t, float alpha = 1.0f);
...
// @p owner's app font at @p height (HellcatLookAndFeel installed), or the
...
{
    if (auto* lnf = dynamic_cast<const HellcatLookAndFeel*> (&owner.getLookAndFeel()))
...
inline const HellcatTheme* themeFor (const juce::Component& owner)
{
    if (auto* lnf = dynamic_cast<const HellcatLookAndFeel*> (&owner.getLookAndFeel()))
        return lnf->getTheme();
    return nullptr;
...
inline const HellcatTheme* themeFor (const juce::LookAndFeel& lnf)
{
    if (auto* p = dynamic_cast<const HellcatLookAndFeel*> (&lnf))
        return p->getTheme();
    return nullptr;
}
...
// HellcatLookAndFeel 14 pt plain font, or the default 14 pt font when another
...
inline juce::Font comboListFont (const juce::Component& owner)
{
    ...
    return hellcat::appFontFor (owner, 14.0f, juce::Font::plain);
}
