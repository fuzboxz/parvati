...
//
// HellcatTheme — a self-contained colour-palette struct plus the 5 built-in
...
// from a single HellcatTheme (switching themes is one pointer change):
//
...
//==============================================================================
// One complete colour palette for the Hellcat UI, organised in three semantic
// layers plus the auxiliary + modulation-routing tokens. The 6 factories in
// HellcatTheme.cpp initialise this struct POSITIONALLY (brace init in field
...
inline const juce::Colour hellcatFallbackAccent { 0xff5b8db8 };

// No-LookAndFeel fallback colours for components that paint in a plain host
// or test harness (no HellcatLookAndFeel installed). These are STATIC safety
...
const HellcatTheme& carbonTheme();     // default = current look (dark / cyan)
const HellcatTheme& midnightTheme();   // dark blue / teal
const HellcatTheme& immutableTheme();  // light gray / magenta (reference adoption)
const HellcatTheme& swedishRedTheme(); // red chassis / grey-black cards / green LCD displays
const HellcatTheme& y2kTheme();         // glossy desktop azure / liquid chrome / candy routing colours
