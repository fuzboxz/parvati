...
// chrome tabs (see HellcatTheme.cpp y2kTheme).
static bool isY2kChrome (const HellcatTheme* theme) noexcept
{
    return theme != nullptr && theme->name == "Y2K";
}

// Text colour for a TOGGLED-ON button (drawn on the accent fill). The default
// is the page token. Two themes carry a mid-luminance base that fails on
// their accent, so they route through their proper text tone: Y2K's gunmetal
// desktop reads 1.64:1 on the LCD green (panel black reads 7.55:1); Swedish
// Red's chassis red is never a text tone (silkscreen off-white reads 4.72:1
// on the deepened signal red). Pairs pinned by tools/check_contrast.py.
static juce::Colour toggledOnTextColour (const HellcatTheme* theme) noexcept
{
    if (theme == nullptr)
        return juce::Colours::black;
    if (theme->name == "Y2K")
        return theme->backgroundPanel;
    if (theme->name == "Swedish Red")
        return theme->textPrimary;
    return theme->backgroundBase;
}

// The Y2K theme's three OFL typefaces, embedded via juce_add_binary_data
// (NAMESPACE HellcatFonts, see CMakeLists.txt). Resolved through the generated
// getNamedResource() instead of #include "BinaryData.h": hellcat_logo_assets
// and hellcat_factory_presets also generate a BinaryData.h, so a direct
// include would be ambiguous (the HellcatLogo pattern). Each Typeface::Ptr is
// built ONCE and cached: createSystemTypefaceFor parses the TTF every call.
namespace HellcatFonts
{
...
{
    int size = 0;
    const char* data = HellcatFonts::getNamedResource (resourceName, size);
...
juce::Font HellcatLookAndFeel::headerFont (float height) const
{
...
juce::Font HellcatLookAndFeel::labelFont (float height, int styleFlags) const
{
...
juce::Font HellcatLookAndFeel::wordmarkFont (float height) const
{
...
juce::Font HellcatLookAndFeel::dataFont (float height, int styleFlags) const
{
...
bool isY2kTheme (const HellcatTheme* t) noexcept { return isY2kChrome (t); }

juce::Colour indicatorFor (const HellcatTheme& t, juce::Colour categoryColour) noexcept
{
...
juce::Colour onCardText (const HellcatTheme* t, juce::Colour themeToken) noexcept
{
...
void paintChromeCard (juce::Graphics& g, const juce::Rectangle<float>& r,
                      float corner, const HellcatTheme* t, float alpha)
{
...
HellcatLookAndFeel::HellcatLookAndFeel()
{
    ...
    // before any component reads it. HellcatEditor overrides this immediately
    // via setTheme(themeManager_.getCurrentTheme()).
    setTheme (carbonTheme());
}

void HellcatLookAndFeel::setTheme (const HellcatTheme& t)
{
    theme_ = &t;
    ...
    // Toggled-on text: the per-theme text tone (see toggledOnTextColour).
    setColour (juce::TextButton::textColourOnId,               toggledOnTextColour (&t));
...
void HellcatLookAndFeel::drawToggleButton (juce::Graphics& g, juce::ToggleButton& button,
...
juce::TextLayout HellcatLookAndFeel::tooltipTextLayout (const juce::String& text, juce::Colour colour) const
{
...
juce::Rectangle<int> HellcatLookAndFeel::getTooltipBounds (const juce::String& tipText,
...
void HellcatLookAndFeel::drawTooltip (juce::Graphics& g, const juce::String& text, int width, int height)
{
...
void HellcatLookAndFeel::drawScrollbar (juce::Graphics& g, juce::ScrollBar& scrollbar,
...
juce::Font HellcatLookAndFeel::appFont (float height, int styleFlags) const
{
    ...
    // "Hellcat" wordmark is also routed through here (bold) so it shares the
    // app typeface.
...
juce::Font HellcatLookAndFeel::getComboBoxFont (juce::ComboBox&)
{
...
juce::Font HellcatLookAndFeel::getComboListFontPublic() const
{
...
juce::Font HellcatLookAndFeel::getTextButtonFont (juce::TextButton&, int)
{
...
juce::Font HellcatLookAndFeel::getPopupMenuFont()
{
...
void HellcatLookAndFeel::getIdealPopupMenuItemSize (const juce::String& text, bool isSeparator,
...
juce::Font HellcatLookAndFeel::getLabelFont (juce::Label& label)
{
...
juce::Font HellcatLookAndFeel::getTabButtonFont (juce::TabBarButton&, float height)
{
...
int HellcatLookAndFeel::getTabButtonBestWidth (juce::TabBarButton& button, int tabDepth)
{
...
void HellcatLookAndFeel::drawTabButton (juce::TabBarButton& button, juce::Graphics& g,
                                        bool isMouseOver, bool isMouseDown)
{
    ...
    // All colours come from the active HellcatTheme tokens (theme_). Horizontal
    // padding is owned by getTabButtonBestWidth (padX 8); the label is centred in
    // the full tab width so the two stay in sync. (Every Hellcat tab bar is
    ...
    // hellcatTabCategoryColourId by SynthWorkspace / GroupPager). Falls back to
    ...
    juce::Colour catColour = button.findColour (hellcatTabCategoryColourId, false);
...
void HellcatLookAndFeel::drawGroupComponentOutline (juce::Graphics& g, int width, int height,
                                                     ...
                                                     juce::GroupComponent& group)
{
    ...
    // hellcat::paintChromeCard — the FX cards and routing bar use the same
    // painter, so every module card matches).
    hellcat::paintChromeCard (g, cardBounds, corner, theme_, alpha);
    ...
    // hellcat_ui_typography_test for the per-theme contrast table).
...
void HellcatLookAndFeel::drawRotarySlider (juce::Graphics& g, int x, int y,
...
void HellcatLookAndFeel::drawLinearSlider (juce::Graphics& g, int x, int y, int width, int height,
...
void HellcatLookAndFeel::drawComboBox (juce::Graphics& g, int width, int height,
                                       bool isButtonDown,
...
void HellcatLookAndFeel::positionComboBoxText (juce::ComboBox& box, juce::Label& label)
{
...
void HellcatLookAndFeel::drawButtonBackground (juce::Graphics& g, juce::Button& button,
                                               ...
                                               bool shouldDrawButtonAsDown)
{
    ...
    // carrying the "hellcatButtonOutlined" component property also
    ...
    // no outline.
    if (! on && button.getProperties().getVarPointer ("hellcatButtonOutlined") != nullptr)
    {
...
void HellcatLookAndFeel::drawHeadingText (juce::Graphics& g, const juce::String& text,
...
void HellcatLookAndFeel::drawTextUncurtained (juce::Graphics& g, const juce::String& text,
                                               const juce::Font& font, juce::Rectangle<float> area,
...
//==============================================================================
// HellcatModuleLamp — the unified module enable/disable indicator (see the
...
void HellcatModuleLamp::paintButton (juce::Graphics& g, bool isMouseOverButton, bool isButtonDown)
{
    ...
    const juce::Colour grey   = t ? t->textDisabled  : hellcat::kFallbackTextDisabled;
...
float HellcatModuleLamp::dotDiameterFor (juce::Rectangle<int> bounds)
{
...
juce::Colour HellcatModuleLamp::resolvedOnColourForTest() const
{
    const HellcatTheme* t = hellcat::themeFor (*this);
    const juce::Colour accent = t ? t->accentPrimary : hellcat::hellcatFallbackAccent;
